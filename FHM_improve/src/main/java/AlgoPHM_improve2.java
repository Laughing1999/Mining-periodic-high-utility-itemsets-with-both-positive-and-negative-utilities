...
public class AlgoPHM_improve2 {
	...
	Map<Integer,Integer> ItemToUtility = new HashMap<>();
	Map<Integer,UtilityListPHM> OneList = new HashMap<>();
	/************  improve2  ***************/
	...
	* the idea is to always reuse the same buffer to reduce memory usage. */
	 ...
	 */
	public void runAlgorithm(int thresholdID,boolean improve,boolean improve2,
		...
		try {
			...
			while ((thisLine = myInput.readLine()) != null) {
				...
				for(int i=0; i <items.length; i++){
					...
					/*********** Improve *************/
					if (improve){
						mmap.put(Integer.parseInt(items[i]),Integer.parseInt(utilityValues[i]));
					}
					/*********** Improve *************/

					/*********** Improve2 *************/
					if (improve2){
						if (ItemToUtility.get(Integer.parseInt(items[i]))==null){
							ItemToUtility.put(Integer.parseInt(items[i]),Integer.parseInt(utilityValues[i]));
						}else {
							int pre_u =  ItemToUtility.get(Integer.parseInt(items[i]));
							ItemToUtility.put(Integer.parseInt(items[i]),Integer.parseInt(utilityValues[i])+pre_u);
						}
					}
					/*********** Improve2 *************/

				}
				mapTidtoMapItemToUtility.put(tid,mmap);
...
//					int remain = remainingUtility; // FOR OPTIMIZATION
		...
		/************** improve2 *************/
		for (UtilityListPHM utilityListPHM:listOfUtilityLists)
		{
			int it = utilityListPHM.item;
			OneList.put(it,utilityListPHM);
		}
		/************** improve2 *************/
		// Mine the database recursively
		phm(itemsetBuffer, 0, null, listOfUtilityLists, minUtility,null);
...
//
//		// Run the algorithm
//		runAlgorithm(improve,input, output, minUtility, regularityThreshold, Integer.MAX_VALUE, 0, Integer.MAX_VALUE);
//	}
...
//		int compare = (int)( mapItemToItemInfo.get(item1).support - mapItemToItemInfo.get(item2).support);
	 ...
	 * @throws IOException
	 */
	private void phm(int [] prefix,	int prefixLength, UtilityListPHM pUL, List<UtilityListPHM> ULs, int minUtility,List<Integer> unpromising)
			throws IOException {
		...
		for(int i=0; i< ULs.size(); i++){
			...
			if(X.sumIutils + X.sumRutils >= minUtility){
				...
				if(patternSize < maximumLength){
					...
					List<Integer> new_unpromising = new ArrayList<>();
					for(int j=i+1; j < ULs.size(); j++){
						UtilityListPHM Y = ULs.get(j);
						/************  improve2  ***************/
						if (Y.sumIutils + Y.sumRutils < minUtility){
							new_unpromising.add(Y.item);
						}
						/************  improve2  ***************/
                        ...
                        /********** improve2 *************/
						boolean flag = true;
						if (unpromising.contains(Y.item)&&ItemToUtility.get(X.item)<minUtility){
							flag = false;
						}
						if (flag){
							...
							if (temp==null){
								if (improve){
									exULs = funcImprove(exULs,Y.item);
								}
							}
							/********** improve *************/
						}else {
							UtilityListPHM temp = OneList.get(Y.item);
							temp.sumIutils = 0;
							temp.sumRutils = 0;
							exULs.add(temp);
					...
					phm(itemsetBuffer, prefixLength+1, X, exULs, minUtility,new_unpromising);
	...
	/********************* improve ***********************/
	private List<UtilityListPHM> funcImprove(List<UtilityListPHM> exULs, Integer item) {
		List<UtilityListPHM> ress = new ArrayList<>();
		for (UtilityListPHM utilityListPHM:exULs){
			UtilityListPHM utilityListPHM1 = new UtilityListPHM(utilityListPHM.item);
			...
			for (Element element: utilityListPHM.elements){
				Map<Integer,Integer> mmap = new HashMap<>();
				mmap = mapTidtoMapItemToUtility.get(element.tid);
				if (mmap.get(item)!=null){
					int rUtil = element.rutils - mmap.get(item);
					Element element1 = new Element(element.tid,element.iutils, rUtil);
					utilityListPHM1.addElement(element1);
				}else {
					Element element1 = new Element(element.tid,element.iutils, element.rutils);
					utilityListPHM1.addElement(element1);
				}
			}
			ress.add(utilityListPHM1);
		}
		return ress;
	}
	/********************* improve ***********************/
...
//		String[] a = String.valueOf(buffer).split(" ");
//		List<Integer> b = new ArrayList<>();
//		for (int s=0;s<a.length;s++){
//			b.add(Integer.parseInt(a[s])) ;
//		}
//
...
//		if (improve){
//			mySql.insertItemsets(String.valueOf(bu),phuiCount,"phm_improve");
//		}else {
//			mySql.insertItemsets(String.valueOf(bu),phuiCount,"phm");
//		}
	 ...
	 * @throws IOException 
	 */
	public void printStats(int thresholdID,String datasetName) throws IOException {
		// print the eucs
		...
		+ optimizationEUCP + optimizationESCP + "=====" + "improve " + improve);
		...
		System.out.println(" call : "             + wocao);

		//把挖掘的一些阈值以及数据集和输出的结果信息存储到SQL中去
		MySql mySql = new MySql();

		if (improve){
			mySql.insertThresAndRes(thresholdID,datasetName,minUtil,minPeriodicity,maxPeriodicity,minAveragePeriodicity,maxAveragePeriodicity,
					totalExecutionTime,maximumMemoryUsage,phuiCount,candidateCount,"phm_improve");

		}else {
			mySql.insertThresAndRes(thresholdID,datasetName,minUtil,minPeriodicity,maxPeriodicity,minAveragePeriodicity,maxAveragePeriodicity,
					totalExecutionTime,maximumMemoryUsage,phuiCount,candidateCount,"phm");
